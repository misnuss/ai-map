...
for r in rows:
    if r.get('country'): by_country[r['country']].append(r)
...
for country, country_rows in by_country.items():
    ...
    for org_name, org_rows in by_org.items():
        ...
        orgs.append({"organization": str(org_name), "type": str(fo.get('organization_type','')),
                     "subtype": str(fo.get('organization_subtype','')), "country": str(country),
                     "continent": str(first.get('continent','')), "last_updated": last_updated,
    ...
    features.append({"type": "Feature",
        ...
        "properties": {"country": str(country), "continent": str(first.get('continent','')),
                       "city": str(first.get('city','')), "org_count": len(orgs), "orgs": orgs}})

geojson = {"type": "FeatureCollection", "features": features}
with open("data.js", "w", encoding="utf-8") as f:
    f.write(f"const COUNTRY_DATA = {json.dumps(geojson, ensure_ascii=False)};")

print(f"✅ {len(features)} Länder, {sum(len(f['properties']['orgs']) for f in features)} Organisationen")
